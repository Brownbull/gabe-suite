...
/* gen-robot.js — THE ROBOT: the endpoint form's parts in request order, each condition with the coverage a test gives it.
   ...
   Leftovers piece 5: "unmeasured" is no longer ONE blanket word. Each unmeasured condition says WHY, by what it is: a switch names the values tests give its
   setting (exercised somewhere — never "this arm proven here"); an effect, a provision or a race says what would prove it (a database read-back). */
...
const ST=Object.fromEntries((fm.settings_tests||[]).map(x=>[x.setting,x]));
const nm=(why,extra)=>Object.assign({state:"unmeasured",cases:[],why},extra||{});
const NM_EFFECT=nm("proven only by reading the database back after the call; the test reading opens response assertions only");
const NM_CATCH=nm("no ending of this door carries this catch's answer, so no test join can reach it");
function nmSwitch(keys){ const vs=keys.map(k=>ST[k]).filter(Boolean);
  const said=vs.map(v=>v.setting+": "+(v.values.length?v.values.join(" | "):"no value set by a test")+(v.default_runs?" (and the default)":"")).join(" · ");
  return nm("no test can be pinned to one arm of a switch here"+(said?"; the values tests give its setting somewhere — "+said:""), {values:vs.map(v=>({setting:v.setting,values:v.values,default_runs:v.default_runs}))}); }
const predKeys=w=>[...new Set((w.branches||[]).flatMap(b=>[...String(b.pred||"").matchAll(/settings\.(\w+)/g)].map(m=>m[1])))];
const L=[];
L.push({ key:"edge", slot:["K3 rate","U3 guards"], name:"EDGE", plain:"the checks every request meets before it reaches this door — the same ones guard every door in the app", stage:"middleware",
  conds: exits.filter(e=>e.phase==="middleware").map(e=>cond(e.id, e.status+" "+(e.detail||""), e.via||e.pred||"", cov(e.tests), "K3", {kind:"exit"}))
    .concat((fm.switches||[]).filter(w=>w.kind==="flag").map(w=>cond(w.id,"flag "+Object.keys(w.settings||{}).join(" · "),"decides whether the limiter runs at all", nmSwitch(Object.keys(w.settings||{})), "U8", {kind:"switch"}))) });
L.push({ key:"gate", slot:["K2 auth","U3 guards","U8 switches"], name:"GATE", plain:"the lock on the door — it decides who may knock, and it writes a row when it lets someone in", stage:"security · dependency",
  conds: exits.filter(e=>e.phase==="security"||e.phase==="dependency").map(e=>cond(e.id, e.status+" "+(e.detail||""), e.via||"", cov(e.tests), "K2", {kind:"exit"}))
    .concat((fm.switches||[]).filter(w=>w.kind==="binding").flatMap(w=>(w.branches||[]).map((b,i)=>cond(w.id+":"+i,"verifier "+b.impl,b.pred||"", nmSwitch(predKeys(w)), "U8", {kind:"switch-arm"}))))
    .concat((fm.auth.provisions||[]).map((p,i)=>cond("prov:"+i,"provision "+p.op+" "+p.table, p.state+" at "+p.committed_at, NM_EFFECT, "K2", {kind:"effect"}))) });
...
L.push({ key:"handler", slot:["U3 guards","U7 refusals","U6 paths","U11 failure"], name:"HANDLER", plain:"the door’s own decisions — its guards, the calls it makes, the arms it takes, the exceptions it catches", stage:"handler",
  conds: handlerExits.map(e=>cond(e.id, e.status+" "+(e.detail||""), e.via||e.pred||"", cov(e.tests), "U7", {kind:"exit"}))
    ...
    .concat((fm.failure.catches||[]).map(c=>{ const ans=c.answers||[]; const e=exits.find(x=>ans.includes(x.status)&&x.phase==="handler"); return cond(c.id,"catch "+(c.types||[]).join(", "), c.outcome+(ans.length?" → "+ans.join(", "):""), e?cov(e.tests):NM_CATCH, "U11", {kind:"catch"}); })) });
...
L.push({ key:"effects", slot:["U9 effects","U12 repeat"], name:"EFFECTS", plain:"the writes the door leaves behind — which survive each ending and which are rolled back", stage:"handler → exit",
  conds: [cond("eff:commit","commit on success", fm.paths.filter(p=>p.kind==="success").map(p=>p.names.drawn+" "+p.effects.n.committed).join(" · "), NM_EFFECT, "U9", {kind:"effect"}),
          cond("eff:rollback","rollback on refusal", fm.paths.filter(p=>p.effects.n.rolled_back).map(p=>p.names.drawn+" "+p.effects.n.rolled_back).join(" · ")||"none", NM_EFFECT, "U9", {kind:"effect"}),
          cond("eff:refusal-writes","a refusal that still writes", refusalWrites.map(f=>f.paths.join(", ")).join(" · ")||"none", refusalWrites.length?{state:"gap",cases:[]}:NM_EFFECT, "U9", {kind:"finding"})]
    .concat((fm.repeat.claims||[]).map((c,i)=>cond("claim:"+i,"claim "+c.table+" ("+c.race+")", (c.idioms||[]).join(", ")+" · "+c.constraint, NM_EFFECT, "U12", {kind:"race"}))) });
