...
from _ep_sectionmap import section_map  # noqa: E402  (the card's ELEVEN ruled blocks, read beside the bench that answers them)
from _ep_fieldlayer import field_layer  # noqa: E402  (D-024 — the field layer's words and the ties a renderer cannot say)
from _ep_mapnav import map_nav  # noqa: E402  (D-024 — the brain map's navigation: groupings, Keep only, moves, open picks)
REPO = HERE.parents[2]
...
def main() -> int:
    ...
    facts["fieldlayer"] = field_layer(facts["sectionmap"])
    facts["mapnav"] = map_nav(facts["sectionmap"])
