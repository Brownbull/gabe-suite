...
WHERE THE TREE COMES FROM, and why it is not recomputed here.
  ...
  PROVES the restatement against that module on every run, then writes the answer into the committed page
  brainmap-endpoint.html as `window.BM_DATA`. A third Python restatement would be a second mapping, and a
  second mapping is an invention — so this module READS that committed tree and verifies it is not stale by
  re-hashing the two inputs the brain map stamped into it (the inventory's bytes, the matrix's cells).
...
def _bm_data(page: Path) -> dict:
    """The brain map's own tree, lifted from the committed page it was generated into."""
    src = page.read_text(encoding="utf-8")
    mark = "window.BM_DATA = "
    i = src.find(mark)
    if i < 0:
        raise Gap("unreadable", "brainmap-endpoint.html carries no window.BM_DATA — the page was not generated by gen-brainmap.js")
    return json.JSONDecoder().raw_decode(src, i + len(mark))[0]
...
def _build() -> dict:
    page, prisms = DC / "brainmap-endpoint.html", DC / "prisms-endpoint.json"
    for f in (page, prisms, WORDS):
        if not f.is_file():
