...
| 10 | Working-tree lab output — 160M gitignored, regenerable (`spike/_build/` 120M · `testing-command-center/output/assets/` 25M · `spike/screenshots/` 9M · `workflow-enhancement/output/site/` 7M · 11 `__pycache__`) | Delete from disk; not a repo trim | **DONE 2026-09-02** — operator confirmed all four; every path `git check-ignore`d + zero tracked files before `rm`; 423M → 262M on disk; `git status` clean before and after; history and clones untouched. Regenerate any lab from its tracked sources. |
