...
> Continue the **5C 3D-trace** spike (graft-adoption arc). The throwaway spike is
> `/home/khujta/gabe-graph-review/spike-kinds/index.html` (+ `3d-bundle.js`, a vendored esbuild bundle of
> THREE + ForceGraph3D + SVGLoader + ConvexGeometry + MarchingCubes). The design record is
> `docs/design/graft-adoption/5C-3D-trace.md` §9a–§9x — read the tail first.
>
> **Verify before changing anything:** `cd /home/khujta/gabe-graph-review/_build && node pw-panel-test.mjs`
> (Playwright over system Chrome, **57/57**). Headless screenshots:
...
## Provenance
- Spec: `docs/design/graft-adoption/5C-3D-trace.md` §9a–§9x (every step, with the why + the fix log).
