...
## SCOPE CONSTRAINT (operator, 2026-08-16) — graft is STRUCTURAL-ONLY; the suite absorbs deep

**We never run `graft init` or `graft build --deep`.** graft is used as a pure structural indexer
(plain `graft build` → `wiring.json`: raw nodes/edges/signatures). Two consequences:

1. **The deep/prose fields are SUITE-produced.** `doc` / summary / crux prose is generated by a SUITE
   process (on the Claude subscription, tied to a beat — e.g. `/gabe-commit` summarizes the diff's
   elements), and written into `archmap` ALONGSIDE the framework fields (`method·path·resp·table·cols·
   fks·uqs·fields·guards`). No graft LLM pass, no graft API key, no `--deep`. In the lineage, the `doc`
   fields move from "graft --deep" to **"suite · deep"**.
2. **Enforcement is SUITE-BUILT, not graft-init.** Because we skip `graft init`, graft's own injection/
   MCP/statusline are NOT installed. The suite owns enforcement — a UserPromptSubmit/PreToolUse hook the
   suite authors (the strongest lever remains injection: run a query over the committed map and inject
   pointers before a grep). graft's `graft ask` CLI (BM25, no init needed) can be the query engine the
   suite's hook points at. ⚠ RIPPLE TO SETTLE: this reverts enforcement from "graft-provides-it-free" to
   "suite-builds-it" — a warn-tier canary hook first (the operator's earlier pick), pointing at the map.
