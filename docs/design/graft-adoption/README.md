...
## SCOPE CONSTRAINT (operator, 2026-08-16) — the ONLY skip is `--deep`

**`graft init` STAYS** — we run it in the projects, so graft's enforcement (UserPromptSubmit injection +
MCP + statusline + `.ignore` re-admit) is installed and FREE. **`graft build` STAYS** (structural). The
ONE thing we skip is the **`--deep` flag**: we do NOT run graft's LLM prose pass.

- **The deep/prose fields are SUITE-produced.** `doc` / summary / crux prose is generated by a SUITE
  process (on the Claude subscription, tied to a beat — e.g. `/gabe-commit` summarizes the diff's
  elements), written into `archmap` ALONGSIDE the framework fields (`method·path·resp·table·cols·fks·uqs·
