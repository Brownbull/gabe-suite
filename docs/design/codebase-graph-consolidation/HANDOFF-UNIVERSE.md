...
0b. **The gap audit is delivered, nothing built** — `universe-build/BACKEND-JOURNEYS.md` §gap audit + artifact Trace Anatomy `a58e4089-789b-4ef6-932a-0c27b7614a30`: seven hop classes the map does not draw (unclaimed files 55/150 · ORM idioms · dependency path · DTO wall · event bus · no-request roots · idempotency). Recommended build order: 1 (config + generalise the census to files) → 2 (ORM symtab) → 4 (serializes edge + touches role-split); costs priced in the memory topic `febe-trace-model`.
0c. **Disclosure Tiers PROPOSAL delivered, not built** — `DISCLOSURE-TIERS.md` + artifact `5e3ef87a-1c2c-48fd-9500-3e91a27e617e`: the path-role ladder (emitter `tier`+`why`, one `__uniLevel` ceiling, the pin as the only exemption; boot L1 = 248/489 today). Twelve decisions await the operator (CONTRACT tier · boot level · fnreads floor · handlers · ghosts · residue · feed shape · depth BFS · per-entity lift · persistence · module home · S11 walkability · FE ladder); build only on "land it".
