...
repl = {
  ...
  176: (176, '<script src="./assets/chip-assets.js"></script>\n<script src="./c4-graph.js"></script>\n<script src="./levels.js"></script>\n<script src="./sim.data.js"></script>\n<script>(function(){ var s=document.createElement("script"); s.src="./workflows.js"; s.onerror=function(){}; document.head.appendChild(s); })();</script>\n<script>(function(){ var s=document.createElement("script"); s.src="./commits.js"; s.onerror=function(){}; document.head.appendChild(s); })();</script>'),   # curated USER WORKFLOWS (window.GABE_WORKFLOWS) + recent COMMITS (window.GABE_COMMITS) — both loaded at runtime with onerror ignored: optional per project, honest-empty when absent, invisible to the static link gate   # sim feed: GABE_SIM (live when a change is in flight · null at rest · file absent = undefined)
...
text = text.replace('preloadBillboards(function(){ build();',
                    'preloadBillboards(function(){ try{ recomputeEX(CFG.entLayout); (window.__uniAssignSplit?__uniAssignSplit():assignSub(CFG.coreByBE||"kind")); recomputeSubAnchors(); }catch(e){} build(); try{ if(window.__uniComputeSolo) __uniComputeSolo(); }catch(e){} try{ __uniSetupOrbit(); }catch(e){} setTimeout(function(){ try{ if(window.__uniSetKindState) __uniSetKindState("function","critical"); }catch(e){} if(window.__uniApplyCapsules) __uniApplyCapsules(); if(window.__uniSetTier){ try{ __uniSetTier(1); }catch(e){} } if(window.__uniCamFit) __uniCamFit(0); }, 400);', 1)   # boot SIMPLIFIED via the T1 (Surface) tier — the tier is the boot-time simplification now that capsules default OFF (operator)
...
text = text.replace(OLD_LINEADD,
  ...
  " var _sr=(typeof CFG!==\"undefined\"&&CFG.selThick!=null)?CFG.selThick:0.2, _so=(typeof CFG!==\"undefined\"&&CFG.selOpacity!=null)?CFG.selOpacity:0.5, _sp=(typeof CFG!==\"undefined\"&&CFG.selPattern)?CFG.selPattern:\"solid\";"   # operator's SETTLED connection look (baked default; panel retired)
  " if(typeof CFG!==\"undefined\"&&CFG.selGlow){ var _gm=new T.MeshBasicMaterial({color:_hlc||0xffffff, transparent:true, opacity:(CFG.selGlowInt!=null?CFG.selGlowInt:0.05), blending:T.AdditiveBlending, depthWrite:false, depthTest:false}); var _gt=new T.TubeGeometry(_crv, Math.max(2,_pts.length-1), _sr*2.6, 6, false); var _gme=new T.Mesh(_gt,_gm); _gme.userData.kind=kind; _gme.userData.__selBaseOp=(CFG.selGlowInt!=null?CFG.selGlowInt:0.05); grp.add(_gme); if(_isSel) window.__uniSelMeshes.push(_gme); }"   # separate GLOW: a wider soft halo tube (operator)
...
# the ⓘ sits RIGHT NEXT TO the "Legend" title (operator) — same placement as the .lgbi info dots beside the
# connector / component / endpoint row labels, not pushed to the far right of the header. (The title line was
# already rewritten upstream to prefix a shape icon, so anchor on the post-patch tail.)
OLD_LGTITLE = '''Legend</b><div class="lgtabs">'''
assert OLD_LGTITLE in text, "legend header title anchor missing"
text = text.replace(OLD_LGTITLE, '''Legend</b><button class="lgref" title="open the full legend reference — every type & badge, with a searchable example"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 11.5v4.5" stroke-linecap="round"/><circle cx="12" cy="8" r="1" fill="currentColor" stroke="none"/></svg></button><div class="lgtabs">''', 1)
