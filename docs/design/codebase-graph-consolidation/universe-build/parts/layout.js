...
window.__BADGE_COL={ method:{GET:"#22c55e",POST:"#3b82f6",PUT:"#f97316",PATCH:"#eab308",DELETE:"#ef4444",BOOT:"#8a8f98"},
                     ...
                     feclass:{connector:"#3b82f6",container:"#8a8f98",leaf:"#22c55e",private:"#8794ab"},   // component CLASS badge (operator palette) — connector=blue data-in · container=gray compose · leaf=green atom · private=gray internal (pure-like)
...
window.__uniLegRef=function(){
  ...
  // a TYPE row: flag toggle · icon · name · description · example. noEx = a PARENT (its examples live on its
  // sub-badges) — no header example, so a container type never borrows a child's node (operator: no overlap).
  function _typeRow(k, noEx){ var Kd=K[k]||{col:"#8794ab",type:k,usage:[0,""]};
    ...
    return '<div class="lrrow"><button class="lrflag'+(_on?" on":"")+'" data-flagk="'+k+'" title="'+(_on?"drawn — click to hide":"hidden — click to show")+'"></button>'
      ...
      +'<i>'+_esc(_desc)+'</i></div>'+(noEx?'':_exChip(_exKind(k)))+'</div>'; }
  ...
  function _feSection(){ var h='<div class="lrsec"><div class="lrsh">FRONTEND</div>';
    ["route","component","hook","type","store","module","web"].forEach(function(k){ if(!K[k]) return;
      ...
      if(_kids){ ["connector","container","leaf","private"].forEach(function(fc){ h+=_badgeRow("feclass", fc, fc, (BD.feclass&&BD.feclass[fc])||"", _exFc(fc)); }); } });
