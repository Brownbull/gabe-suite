...
window.__uniBgClick=function(ev){ try{ if(!ev || ev.clientX==null) return;
    ...
    if(window.__uniStubs && window.__uniStubs.length){ var gbest=null;
      ...
      if(gbest && window.__uniReveal){ var _gid=gbest.st.hid; __uniReveal(_gid);   // reveal the cluster/entity IN PLACE, then SELECT the node (operator: panel + trail)
        var _gn=NIDS[_gid]; if(_gn){ SEL={kind:"node",data:_gn}; try{ showPanel(_gn); }catch(e5){} try{ if(typeof refreshEncSel==="function") refreshEncSel(); }catch(e6){} try{ if(window.__uniHLSelect) __uniHLSelect(_gn); }catch(e7){} } } }
  }catch(e){} };
...
window.__uniWireTopbar=function(){
  ...
  if(!window.__uniFly){ window.__uniFly=1; var FK={};
    ...
    window.addEventListener("keydown", function(e){ if(!_flyOK(e)) return; var k=e.key.toLowerCase();
      ...
      else if(e.altKey&&(k==="a"||k==="d")){ e.preventDefault(); try{ if(typeof _walkGo==="function") _walkGo(k==="d"?1:-1); }catch(_we){} }   // Alt+A/D = trail prev/next (operator)
...
window.__uniFleetToggle=function(ent, sub, col){ try{
  ...
  if(sub!=null && ent!=="*"){ var key=ent+"|"+sub, sv=UNIVIS.sub[key]||(UNIVIS.sub[key]=Object.assign({},_VISDEF));
    ...
    sel.forEach(function(e){ UNIVIS.ent[e][col]=on?1:0;
      _entSubKeys(e).forEach(function(k){ (UNIVIS.sub[k]||(UNIVIS.sub[k]=Object.assign({},_VISDEF)))[col]=on?1:0; }); }); }   // cascade to EVERY cluster (init the missing ones) — no cluster is left "waiting on" under a hidden entity (operator)
