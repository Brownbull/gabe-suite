...
var _ringTexCache={};
function _ringTex(pat, thick){ pat=pat||"spinner"; thick=thick||4; var key=pat+"|"+thick; if(_ringTexCache[key]) return _ringTexCache[key];
  var cv=document.createElement("canvas"); cv.width=cv.height=128; var c=cv.getContext("2d"); c.strokeStyle="#fff"; c.lineCap="round"; c.lineWidth=thick;
  if(pat==="solid"){ c.beginPath(); c.arc(64,64,50,0,6.2832); c.stroke(); }
  else if(pat==="dashed"){ c.setLineDash([15,11]); c.beginPath(); c.arc(64,64,50,0,6.2832); c.stroke(); }
  else { c.beginPath(); c.arc(64,64,50,-Math.PI*0.5,Math.PI*1.15); c.stroke(); }   // spinner (gapped arc) — no "dotted" (operator)
