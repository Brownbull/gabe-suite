...
function _numBadgeSprite(num, col, size, cur){ var cv=document.createElement("canvas"); cv.width=cv.height=128; var c=cv.getContext("2d");
  var C=col||"#9ecbff";
  c.beginPath(); c.arc(64,64,54,0,Math.PI*2); c.fillStyle="#0b0f18"; c.globalAlpha=cur?0.9:0.82; c.fill(); c.globalAlpha=1;   // dark disc so the number reads over the bright scene glows
  c.lineWidth=cur?11:8; c.strokeStyle=C; c.stroke();                                                                        // the ENTITY-colour ring (trail coloring); current step = thicker
  var t=String(num); c.font="800 "+(t.length>2?42:t.length>1?56:70)+"px "+curFont(); c.textAlign="center"; c.textBaseline="middle";
  c.lineWidth=4; c.strokeStyle="#0b0f18"; c.strokeText(t,64,69); c.fillStyle=cur?"#ffffff":C; c.fillText(t,64,69);           // number in the entity colour (current = white), dark outline for contrast on any hue
  var s=new T.Sprite(new T.SpriteMaterial({map:new T.CanvasTexture(cv), transparent:true, depthWrite:false, depthTest:false})); s.scale.set(size,size,1); return s; }
...
window.__uniDrawJourneyNums=function(){ try{
  ...
  WALK.steps.forEach(function(id, i){ var n=(typeof _fnById==="function")?_fnById(id):(typeof NIDS!=="undefined"&&NIDS[id]); if(!n) return;
    ...
    var s=_numBadgeSprite(i+1, col, cur?18:14, cur); s.position.set(p.x,p.y,p.z); s.userData.nid=id; s.raycast=function(){}; grp.add(s); }); }catch(e){} };
...
window.__uniHLTick=function(){                                                  // follow the sim every cluster tick (focus keeps origin halos)
  ...
  var jg=window.__uniJrnNumGroup; if(jg && jg.children.length) jg.children.forEach(function(s){ if(!s.userData.nid) return; var p=_npos[s.userData.nid]; if(p) s.position.set(p.x,p.y,p.z); }); };   // journey step-number badges track the sim too
