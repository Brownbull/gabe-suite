...
function __uniSetupOrbit(){ var g=document.getElementById("g"); if(!g || g.__orbitBound) return; g.__orbitBound=true; var drag=null;
  g.addEventListener("pointerdown", function(ev){ if(ev.button!==0) return;
    try{ var cam=Graph.camera(), ctrls=Graph.controls(); if(!cam||!ctrls||!ctrls.target) return;
      ...
      drag={ P:P, lx:ev.clientX, ly:ev.clientY }; ctrls.enabled=false;   // custom rotation owns this drag
  ...
  window.addEventListener("pointerup", function(){
    if(drag){ var ctrls=Graph.controls(); drag=null; if(ctrls){ ctrls.enabled=true; try{ ctrls.update(); }catch(e){} } }
...
window.__uniAddLayoutTab=function(){ var cfg=document.getElementById("cfg"); if(!cfg) return;
  ...
  if(head && !head.querySelector("#motionBtn")){ var mo=document.createElement("button"); mo.className="cfgmin"; mo.id="motionBtn";
    ...
    mo.onclick=function(){ ANIM.all=!ANIM.all; mo.textContent=ANIM.all?"⏸":"▶"; mo.classList.toggle("on",!ANIM.all); };
  ...
  var pillHTML=function(grp, opts, cur){ var s='<div class="pill" data-grp="'+grp+'">';
    opts.forEach(function(o){ s+='<button data-v="'+o.v+'"'+(o.v===cur?' class="on"':'')+'>'+o.t+'</button>'; }); return s+'</div>'; };
  ...
  // ── UNIVERSE pane: container · sub-cluster + radius · cluster transparency · stars/transports · layout ──
  ...
  if(G.universe){ uni.appendChild(G.universe);   // add a transport-speed slider under the Stars/Transports icons
    var tsp=mk("cfgrow"); tsp.style.marginTop="6px";
    tsp.innerHTML='<span class="rlbl" style="width:auto;font-size:10px;text-transform:none;letter-spacing:0">transport speed</span>'
      +'<input type="range" class="rng" id="trSpeedRng" min="0.05" max="1.2" step="0.05" value="'+((typeof INTC!=="undefined"&&INTC.speed)||0.3)+'">';
    G.universe.appendChild(tsp); }
  ...
  layWrap.innerHTML=
    ...
    + pillHTML("lineStyle",[{v:"straight",t:"Straight"},{v:"curved",t:"Curved"}], CFG.lineStyle)+'</div>'
  ...
  bar.innerHTML='<button class="cfgtab on" data-pane="planets">Planets</button><button class="cfgtab" data-pane="universe">Universe</button>';
  body.appendChild(bar); body.appendChild(pl); body.appendChild(uni);
  bar.querySelectorAll(".cfgtab").forEach(function(t){ t.onclick=function(){ var pane=t.getAttribute("data-pane");
    ...
    pl.style.display=(pane==="planets")?"":"none"; uni.style.display=(pane==="universe")?"":"none"; }; });
  // wire the NEW controls only (the moved spike pills keep their wireCfg listeners)
  [pl, uni].forEach(function(pane){ pane.querySelectorAll(".pill[data-grp]").forEach(function(p){ var grp=p.getAttribute("data-grp");
