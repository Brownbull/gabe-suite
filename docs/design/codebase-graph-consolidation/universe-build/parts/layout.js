...
window.UNICTL={ invert:false, selPivot:true, camMode:"tumble" };   // camMode: tumble · joystick · arcball · look — the left-drag scheme
...
function __uniSetupOrbit(){ var g=document.getElementById("g"); if(!g || g.__orbitBound) return; g.__orbitBound=true; var drag=null;
  ...
  window.addEventListener("pointermove", function(ev){
    if(!drag && (ev.buttons&1) && (ev.buttons&2)){ drag=_rigStart(ev.clientX, ev.clientY); if(drag) drag.btn=0; if(!drag) return; }   // left joined a right-pan mid-stream
    if(!drag) return;
    try{ var cam=Graph.camera(), ctrls=Graph.controls();
      ...
      if(ev.buttons&2 && drag.btn!==1){                                     // CHORD (left+right): BOTH at once — the rig DRIFTS (half-strength pan)
      ...
      var mode=(drag.btn===1)?"tumble":(UNICTL.camMode||"tumble");           // middle-orbit always tumbles around the selection
  ...
  setInterval(function(){                                                    // JOYSTICK tick: offset from the CLICK ANCHOR = angular velocity —
    try{ if(!drag || drag.btn===1 || (UNICTL.camMode||"tumble")!=="joystick" || drag.cx==null) return;   // hold farther = turn faster (the WoW feel)
...
window.__uniBuildCtrl=function(){ if(document.getElementById("ctrlp")) return;
  ...
  p.innerHTML='<div class="cfghead" id="ctrlphead"><span class="cfgtitle">'+(typeof ico==="function"?ico("target",13):"")+'Controls</span>'
    ...
    +'<div class="ctlrow"><span class="ctll">camera</span><select id="ctlCam" title="how the LEFT drag rotates">'
    +'<option value="tumble">Tumble — drag = turn (stock)</option>'
    +'<option value="joystick">Joystick — hold offset = keep turning (WoW)</option>'
    +'<option value="arcball">Arcball — virtual trackball</option>'
    +'<option value="look">Look — first-person, turn in place</option></select></div>'
    +'<div class="ctlrow ctltog"><button class="fltog'+(UNICTL.invert?" on":"")+'" id="ctlInv" title="flight-style: drag up looks DOWN (flips the vertical rotation axis only — buttons stay the same)"></button><span class="ctll">invert vertical (flight-style)</span></div>'
...
window.__uniAddLayoutTab=function(){ var cfg=document.getElementById("cfg"); if(!cfg) return;
  ...
  layWrap.innerHTML=
    ...
    + pillHTML("entLayout",[
        ...
        {v:"ring",t:"Ring",ti:"a coupling-ordered circle — even spacing, separation by construction"}], CFG.entLayout)+'</div>'
