...
function _wfCollect(bk){ var W=window.GABE_WORKFLOWS; if(!W||!W.length) return [];
  ...
  return W.map(function(w,i){ var steps=[], ents={}, miss=[], writes=0, epn=0;
    ...
    return { cid:"wf:"+i, wf:true, corpora:{workflow:1}, corpus:"workflow", agg:false, e2e:false,
      ...
      level:w.level||0,                                        // dev-onboarding expertise tier (1/2/3; 0 = unleveled → "other")
...
function _jrnRow(j){ var badge=j.bk?j.method:(j.wf?(j.epn+" ep"+(j.miss.length?(" · "+j.miss.length+" unmapped"):"")):(j.agg?"agg":j.cid));
  ...
  +(j.wf&&j.level&&_JRNLEVELS[j.level]?('<span class="jrnlvl l'+j.level+'" title="tier '+j.level+' · '+_JRNLEVELS[j.level].label+'">'+_JRNLEVELS[j.level].ico+'</span>'):"")
...
function _jrnGroupsHTML(){ var sel=window.__uniJrnKind, bySpan=function(a,b){ return (b.ents.length-a.ents.length)||(b.carriers.length-a.carriers.length); };
  ...
  if(sel==="wf"){                                              // WORKFLOWS group by DEV-ONBOARDING LEVEL (operator): a new dev reads ①→②→③ top-down
    ...
    Object.keys(byLvl).map(Number).sort(function(a,b){ return a-b; }).forEach(function(L){
      var m=_JRNLEVELS[L]||{label:"other",hint:"unleveled",ico:_lvlBars(0)}, key="lvl"+L, cl=!!window.__uniJrnCollapse[key];
      h+='<div class="jrngrp jgcl jglvl'+(cl?" cl":"")+'" data-ge="'+key+'" title="'+m.hint+'"><span class="jcar">'+_JCHEV+'</span><span class="jglvlico">'+m.ico+'</span>'+m.label+' · '+byLvl[L].length+'</div>';
      if(!cl) byLvl[L].forEach(function(j){ h+=_jrnRow(j); });   // curated order within a tier — never re-sorted
    });
    return h;
  }
