...
| B13 | **Colour the map's connections by path kind — golden path vs refusal vs exception** | Operator, 2026-09-16, on reading the §A4 acceptance sheet: the element forms now carry, per endpoint, one ordered path per exit with its kind (success · refusal · framework · uncaught), every `catch` on the chain and the write buckets — the fields a renderer needs to draw the expected route in one colour and every other ending in others. A rendering choice for Phase 6, decided by SEEING (never from text). | `forms.json` `paths[]` (`exit.kind`, `chain[].kind == catch`, `effects.{committed,rolled_back,…}`), `frontend.pieces[guard].chain`; the universe station already draws bridge wires per edge kind. | **DEFERRED** until Phase 6's panels exist to colour |
| B14 | **Common vs specialist pieces — a wiring fingerprint compared across apps** | Operator, 2026-09-16: the forms describe each piece's INTERNAL wiring (which stations it fills, with which idioms); compared across applications that becomes "this shape is battle-tested everywhere" vs "only here" — a library of standard components a developer could navigate like a game, and the honest answer to "did gustify/gastify go far past standard pieces?". Needs a corpus: the four targets (gustify · gastify · tier3 · keypro) are one. | Per-arm `stats.idioms` (contract `begin_nested`, `get-or-create`; frontend `LIBRARY_DEFAULTS`; `CONTRACT.rate_idioms`; `F.KINDS` slot registry), `functions{}` reach depths, `frontend.idioms.unknown`. First probe = a READ-ONLY census over the four existing feeds, no generator change. | **DEFERRED** until the §A4 close; the census is the trigger's first move |
| B15 | **`tests/center` rowmark-e2e flakes around the UTC date rollover** | The e2e badges "the appended row" against a `T−1m` history entry; it failed twice on 2026-09-16 between ~19:30 and ~22:00 local (−03) — `STRAY badge in test-corpora.html … 2026-09-17 … 2 stray badge(s) of 5` — and passed on every re-run outside that window. The page renders the date in UTC (already 09-17) while the appended row's key is local-dated; a row whose date string moved re-badges. Turns the doctor red for an hour a day. | `$SP/center-1.log` vs `center-2.log` same tree, 30 min apart: 175/3 then 178/0; write_facts at 22:4x local recorded it RED. | **OPEN** — pin the digest's date to one clock (UTC) or exclude the date cell from the row key |
