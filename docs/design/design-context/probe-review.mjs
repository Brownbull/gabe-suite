...
{ const bad = await p.evaluate(() => { const ruled = new Set(window.REVIEW_DATA.decisions.filter(d => d.ruled).map(d => d.id));
    return [...document.querySelectorAll('.dec, .nrow')].filter(e => !ruled.has(e.dataset.dec)).filter(e => e.querySelectorAll('[data-mine="true"]').length !== 1 || e.querySelectorAll('[aria-pressed="true"][data-v]').length !== 0).map(e => e.dataset.dec || e.dataset.norm); });
  ok(bad.length === 0, 'untouched, every card and line shows exactly ONE dashed pick and nothing filled', bad.join(',')); }
{ const mism = await p.evaluate(() => window.REVIEW_DATA.decisions.filter(d => !d.ruled).filter(d => { const c = document.querySelector('.dec[data-dec="' + d.id + '"]'); const vs = c ? [...c.querySelectorAll('button.imp[data-v]')].map(x => x.dataset.v) : [];
    const dashed = c && c.querySelector('button.imp[data-mine="true"]');   /* a missing card or a missing dashed button is a FAILED check, never a crash */
...
{ const left = await p.$$eval('.dec, .cap, .lede, .short', els => els.map(e => e.textContent).filter(t => /\{\w+\}/.test(t)));
...
{ const bad = await p.evaluate(() => window.REVIEW_DATA.decisions.filter(d => !d.ruled).filter(d => { const c = document.querySelector('.dec[data-dec="' + d.id + '"]'); if (!c) return true;
  ...
  const d = D.decisions.find(x => x.nature === 'code' && x.options.length >= 2), other = d.options.find(o => o[0] !== d.mine)[0], sel = `.dec[data-dec="${d.id}"]`;
  await p.click(`${sel} button.imp[data-v="${other}"]`); await p.waitForTimeout(80);
  ok(await p.$eval(`${sel} .imp[data-v="${other}"]`, e => e.dataset.on) === 'true' && await p.$eval(`${sel} .imp[data-v="${d.mine}"]`, e => e.dataset.on) === 'false', 'an option is its own button: choosing it lights its impact and unlights my pick');
  await p.click(`${sel} button.imp[data-v="${other}"]`); await p.waitForTimeout(80);
  ok((await p.$$('#norm-impacts .imp')).length === 3, 'the three verdicts on a norm line say what each sets in motion, once'); }
/* D-017 — facts about the map, and choices inside the framework, are folded away; the lines the lab does not draw are too */
{ const F = await p.evaluate(() => { const D = window.REVIEW_DATA, vis = e => !!(e && e.offsetParent !== null);
    return { map: D.decisions.filter(d => d.nature === 'map').map(d => d.id), inFold: [...document.querySelectorAll('#cards-map .dec')].map(e => e.dataset.dec), foldHidden: document.getElementById('cards-map').hidden,
      codeVisible: D.decisions.filter(d => d.nature === 'code').every(d => vis(document.querySelector('.dec[data-dec="' + d.id + '"]'))), mapVisible: D.decisions.filter(d => d.nature === 'map').some(d => vis(document.querySelector('.dec[data-dec="' + d.id + '"]'))),
      drawnRows: [...document.querySelectorAll('.nrow')].filter(vis).length, drawn: D.norms.filter(n => n.drawn).length, moreOpen: [...document.querySelectorAll('.dec .more')].filter(m => !m.hidden).length,
      ruled: D.decisions.filter(d => d.ruled).map(d => d.id) }; });
  ok(F.map.length >= 1 && F.map.slice().sort().join() === F.inFold.slice().sort().join() && F.foldHidden && !F.mapVisible && F.codeVisible, 'a decision about the map sits in the folded section, out of sight, and every decision about the code stays in front of him', F.map.join(','));
  ok(F.drawnRows === F.drawn && F.drawn < D.norms.length, 'only the norm lines the lab really draws are open; the rest are folded', F.drawnRows + ' shown · ' + F.drawn + ' drawn · ' + D.norms.length + ' in all');
  ok(F.moreOpen === 0, 'every card opens on its short form — the full story sits behind "more"', String(F.moreOpen));
  await p.click('#s6-fold'); await p.waitForTimeout(120);
  ok(await p.$eval('#cards-map', e => !e.hidden) && (await p.$$eval('#cards-map .dec', els => els.every(e => e.offsetParent !== null))), 'the folded decisions open on one click, all of them');
  ok(F.ruled.length >= 1 && (await p.$$eval('#cards-map .dec[data-dec="' + F.ruled[0] + '"] button.imp', els => els.length)) === 0 && /ruled by you/i.test(await p.$eval('#cards-map .dec[data-dec="' + F.ruled[0] + '"]', e => e.textContent)), 'a decision he already ruled in the chat says so and offers no options', F.ruled.join(','));
  await p.click('#s6-fold'); const firstCode = D.decisions.find(d => d.nature === 'code').id; await p.click(`.dec[data-dec="${firstCode}"] [data-more]`); await p.waitForTimeout(80);
  ok(await p.$eval(`.dec[data-dec="${firstCode}"] .more`, e => !e.hidden && e.querySelectorAll('.where li').length >= 1), '"more" opens the full story and where to check it'); await p.click(`.dec[data-dec="${firstCode}"] [data-more]`); }
const text = () => p.$eval('#out', e => e.value);
{ const t = await text(), nRuled = D.decisions.filter(d => d.ruled).length;
  ok(t.startsWith('REVIEW · leftovers · ' + D.hash) && new RegExp('\\n' + nRuled + ' yours · ').test(t) && D.decisions.filter(d => !d.ruled).every(d => t.includes('\n' + d.id + ': ' + d.mine + ' (my pick, not ruled)'))
    && D.decisions.filter(d => d.ruled).every(d => t.includes('\n' + d.id + ': ' + d.ruled.value + ' (yours, ruled in the chat')) && t.indexOf('LEFT TO MY PICK BY YOUR RULE') > t.indexOf('PIECE 11'),
    'untouched, the text lists every decision as my pick, what he ruled in the chat as his, and the folded ones under their own heading', t.split('\n')[1]); }
{ const d = D.decisions.find(x => x.nature === 'code' && x.options.length >= 2), other = d.options.find(o => o[0] !== d.mine)[0], sel = `.dec[data-dec="${d.id}"]`;
  await p.click(`${sel} button.imp[data-v="${other}"]`); await p.waitForTimeout(80);
  ok(await p.$eval(sel, e => e.dataset.state) === 'yours' && (await text()).includes(`\n${d.id}: ${other} (yours, I picked ${d.mine})`) && new RegExp('^' + (D.decisions.filter(x => x.ruled).length + 1) + ' of ').test(await p.$eval('#count', e => e.textContent)), 'ruling a card fills the button, marks the card yours, and the text says what I had picked', d.id);
  await p.click(`${sel} .verdict .btn.nb`); await p.fill(`${sel} textarea.note`, 'a note\nin two lines'); await p.waitForTimeout(80);
  ...
  await p.click(`${sel} button.imp[data-v="${other}"]`); await p.waitForTimeout(80);
  ok(await p.$eval(`${sel} button.imp[data-v="${d.mine}"]`, e => e.dataset.mine) === 'true', 'pressing the same option again hands the card back to my pick'); }
...
ok(new RegExp('\\n' + D.decisions.filter(d => d.ruled).length + ' yours · ').test(await text()), 'Clear everything, pressed twice, hands every line back');
