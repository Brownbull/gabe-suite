...
> **Superseded 2026-09-20 by the review page** — `review-leftovers.html` (generated by `gen-review.js`; published at https://claude.ai/artifact/TYtFyh7fZcDbeZzYcJFuwZ). The operator asked for an artifact with options, picks, pictures and one copy button instead of a text plan. This file stays as the first draft; two of its claims were wrong and are corrected on the page (the lab does NOT already say "endpoint" everywhere; "fifteen tables" was 2 of 15).
