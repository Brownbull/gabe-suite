...
| what the screen does on this ending (message · retry · redirect) | relation | the routing half is measured (piece 10, lab half): 6 of 80 endpoints have client code that reads their failures; 47 endings routed — 14 to a branch of their own, 33 to the general case, 10 share a branch with another ending; retry is off on 32 endpoints; a success refreshes 0 · 0 · 6 query keys on 15 endpoints; 0 · 0 · 2 guards read what it refreshes on 3. What the branch DOES (the message, a call, the element) waits for the generation part | **3** (proposed) | a retry repeats the whole route and a redirect leaves the screen | with code | relation | mid — **feed gap: needs a new generation part (piece 10)** |
