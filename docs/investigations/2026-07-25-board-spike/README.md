...
Wire it into the generators as `board.html`: the four framings, State as the landing view, the
phase strip on top, the three old lanes retired. The extractor here is the working reference —
it reads both twins with no per-project branching.
