...
| `/gabe-execute` | ![gabe-execute](assets/icons/cmd-execute.png) TASK RECORD trailer + REUSE obligation — every checkpoint commit carries `Task:`/`Cases:`/`Class:` lines validated by `/gabe-commit` (the printed contract retired 2026-08-07); the `Cases:` line ties the work to the C-ids it must turn green; "verification ✅" is unprintable without an executed command in front of it. |
