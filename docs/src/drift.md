...
**The mechanism that now catches it:** the execute task record. Before any code is written for a task that names an external reference, the deliverable class — `red`, `guard`, `wiring`, `growth` — is named in the checkpoint commit's `Class:` trailer (validated by `/gabe-commit`'s `checkpoint-trailer.sh`), the task states its done-when acceptance against the named reference, and the acceptance check is a same-viewport side-by-side diff, never "renders and tests are green." If the planned work is a cheaper class than the instruction implies, that is a stop-and-ask, not a judgment call. (This began as a *printed* TASK CONTRACT block; it went 0-for-19 across two audited cycles and was retired 2026-08-07 — the record moved to where a machine already looks, the commit message, and the obligations survive as prose in the execute spec.)
