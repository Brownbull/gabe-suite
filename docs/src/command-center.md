...
The Code tab runs two **scoped lenses in one dialect**: the DATA-MODEL lens (every documented class) and the FUNCTIONS lens (every def in the mapped backend files). Each renders the same way — icon tags on the name cell, filter chips (base · ≈similar · god), a **two-bar usage cell** (api = endpoint touches + FK in · internal = mapped files referencing), a titled row detail (usage receipts as tables, calls, signature/structure), and a first-class **candidates section** where the machine *names* merge / split candidates and judgment rules them. The signals ride `archmap.json` as `model_insight` and `function_insight` — computed in the same build pass off the same cached parses, no extra step, no authored input.

Three honesty rules carry the lenses:

- **Two bars, because one lies — and neither is a verdict (R10).** A class or function with zero endpoint touches can still be load-bearing service vocabulary, which is why the violet internal bar exists. But zero on *both* axes is not death either: the refs scan reads config-mapped Python only, so a zero says as much about the config as about the code. Audited across both twins, the retired `orphan` flag was **~94% false positive** — 126 of 134 flagged symbols provably referenced, 7–8 genuinely dead across ~15,500 non-test callables. The bars now REPORT: a zero reads *"no indexed usage — go check"*, never *"dead"*. One unmapped file falsifies a zero; nothing falsifies a count.
- **Candidates stand on corpus-complete signals.** MERGE (structural / identifier similarity) and SPLIT (size) are true regardless of what lies outside the map — absence is not. Both sections state their floor: top-1 nearest neighbour only, above the similarity line only, Python only, mapped files only. Every one of those is a way to MISS a pair, never a way to invent one, so the page is a **floor, not a census** — and an empty candidates table means the mapped set is clean, not the repository.
