...
6. Seed `.gitignore` with `.kdbp/reviews-archive/`, `.kdbp/.push-gate-ok`, `docs/site/center/inflight.{json,js}`, and `docs/site/center/sim.data.js` (idempotent, grep-before-append per entry) — local-only runtime artifacts (the last two are beat-tail projections whose `head` churns every commit) that must never ride a commit.
