...
2. **Break into phases** — Specific, actionable steps. Each phase has:
   ...
   - **Entities (command-center projects only, ruling 2026-08-07)** — PROPOSE the center entity slug(s) this phase's work touches by matching the phase's Scope paths against `center.config.json` `entities{}.code` globs (and the archmap): `mcp__gabe-map__owner_of` performs that join for a FILE path — its archmap owners, `config_glob_owners` (entity slugs, never the pattern) and a `note` NAMING where the map is blind — and the glob join is a pure pattern match, so it answers for a path that does not exist yet, which is the case that matters here. A glob-shaped Scope entry is passed as the concrete files it will create (or one representative path under it); pass a DIRECTORY only for an area that already exists — that branch tallies `owners` over files ALREADY mapped plus `unclaimed_in_census` and skips the config-glob join, so a brand-new area answers an empty tally that is not a `none`. Where the server is not registered, perform the same join by hand — match each Scope file/glob against the `entities{}.code` globs and the archmap's per-entity files, and cite the blind spots the same way. Present the proposal with the plan and let the operator confirm or correct it. A phase whose work maps to no registered entity declares `none — <reason>` — an honest blank, NEVER a guessed slug (the board's own attribution law: path-derived or absent). Declared entities are what the center's in-flight view can trust BEFORE any code exists; `/gabe-review` verifies declared-vs-touched drift after.
...
<!-- Center column (OPTIONAL, command-center projects only): add `| Center |` after Push; auto-ticked ✅ by /gabe-cc-update when it covers the shipped phase. Absent → /gabe-next skips it (treated as ✅). -->
...
<!-- - **Proof:** line — OPTIONAL .md documentation of the phase's runtime-evidence record. The mirror's `proof` FIELD is written by /gabe-execute directly into PLAN.json (execute-spec); regen-mirror.py prefers this bullet when present and PRESERVES the mirror's existing value when absent (regeneration never wipes recorded evidence to null — a null proof would also blind the plan-proof-guard). Proof never lives in the table -->
...
- Cell tokens mirror the table glyphs 1:1: ⬜ `todo` · 🔄 `in_progress` · ✅ `done` · ⏸ `deferred` · ⚰️ `obsolete`. One render-only glyph exists: `—` in a Red cell means the phase is outside the red beat's window — it predates the beat, OR (R1 as amended) was already executing (🔄) when the column was adopted — and maps to NO token: the mirror omits the `red` key for that phase (readers treat a missing key as not this phase's concern; verified `next.mjs` behavior).
...
Compose the row with `mcp__gabe-kdbp__pending_row_preview` — one `flag` object (`description:` `Plan deferred: "[goal]"`, `source: gabe-plan`, `file:` the archive path, `scale:` the maturity, `severity`/`impact` as asked) — which returns the row in the file's OWN column order, with the next `P[N]` minted over live AND archived rows. Then set the two cells the preview fixes differently, so the row matches the canonical one below: `Status` → `open` (the preview leaves it blank, and the review re-surface selects on `Status: open`) and `Verified` → `—` (it stamps `@<sha> <date>`, but a plan deferral is prose, not a finding derived against a checked tree — gate-spec Step 6's rule); its `Times Deferred` `1` already matches. Write it to `.kdbp/PENDING.md` with the harness Write/Edit so the D7 hooks see the write (the preview writes nothing), or through the writer the preview names where the project carries `scripts/disposition.py --defer` (the disposition contract's sanctioned writer — plan is one of its six named adopt sites). Where the server is unregistered, author the canonical row below by hand:
...
- **`update`**: Scope fence (check before ANY edit):
  ```
  1. NEVER edit Red/Exec/Review/Commit/Push/Center cells — those belong to their commands (Red → /gabe-red, Center → /gabe-cc-update). `update` MAY add the `Red` or `Center` column itself when a project adopts the red beat / a command center — that is a schema edit, not a cell tick. Red seeding (ruling R1, amended): seed ⬜ ONLY on phases whose Exec is ⬜ (todo); ✅ AND 🔄 rows render `—` and their PLAN.json phase OMITS the `red` key. Never ⬜ on shipped OR in-progress rows: shipped, the router re-opens the work and demands a fake red (D5 violation); in-progress, the exec-resume branch wins today but the seeded ⬜ becomes unreachable debt the sweep flags after Exec completes — the same fake-red demand, one phase later. Never ✅ (a Red ✅ without a cases record is a lie the plan-proof-guard blocks). Center seeds ⬜ everywhere as before: shipped phases genuinely owe coverage.
...
- Each writing command's own spec defines its row content; every writer appends exactly ONE row per checkpoint. Where the `Entry` is one the preview accepts (`PLAN RED EXEC REVIEW COMMIT PUSH CENTER HANDOFF SCOPE ASSESS`), compose the row with `mcp__gabe-kdbp__ledger_row_preview` (`entry` + `theme` required; it returns the row in THIS file's own column order, with the `Gates / results` cell copied verbatim) and write it with the harness Write/Edit — never through a tool — so the D7 hooks see the write. A satellite tag the preview does not carry (`ALIGN`, `MOCKUP` — it stops on them), or an unregistered server → author the row against the header above.
