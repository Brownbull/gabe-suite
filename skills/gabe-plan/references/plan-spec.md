...
2. **Break into phases** — Specific, actionable steps. Each phase has:
   ...
   - **Entities (command-center projects only, ruling 2026-08-07)** — PROPOSE the center entity slug(s) this phase's work touches by asking `mcp__gabe-map__owner_of` with the phase's Scope paths (a glob-shaped Scope entry: pass its directory) — a FILE path returns its archmap owners, its `center.config.json` `entities{}.code` glob owners, and a `note` NAMING where the map is blind; a DIRECTORY returns the per-entity `owners` tally over its mapped files plus `unclaimed_in_census`, and an empty tally with unclaimed files is the reason an honest `none` cites (pass the files themselves when the `note` matters); present the proposal with the plan and let the operator confirm or correct it. A phase whose work maps to no registered entity declares `none — <reason>` — an honest blank, NEVER a guessed slug (the board's own attribution law: path-derived or absent). Declared entities are what the center's in-flight view can trust BEFORE any code exists; `/gabe-review` verifies declared-vs-touched drift after.
