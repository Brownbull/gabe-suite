...
Wire laws (design record §5, verified against Claude Code 2.1.257/258):
...
- nothing heavy before initialize is answered; the map loads lazily on the first call.
"""
...
class Server:
    ...
    def on_response(self, msg: dict):
        if str(msg.get("id", "")).startswith("srv-") and "result" in msg:
            ...
            if roots:
                self.roots = roots
                mq.log("roots: %s" % roots)
        elif "error" in msg:
            mq.log("client error reply: %s" % msg.get("error"))
    ...
    def on_request(self, msg: dict):
        ...
        if method == "initialize":
            ...
            return self.wire.send(_result(id_, {
                "protocolVersion": ver,
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "gabe-map", "version": VERSION},
    ...
    def handle_line(self, raw: bytes):
        ...
        try:
            ...
            mq.log("unparseable line skipped (%d bytes)" % len(line))
            return
        if isinstance(msg, list):                       # batches: 2025-03-26 only — not accepted here
            mq.log("batch ignored")
            return
