...
1. **Read the phase** — description, tier, `proof_type` — from `mcp__gabe-kdbp__phase_context`: the phase record, its row states, any `Cases:`/`Reach:` already recorded, the `details_excerpt` and the Verify Commands in one answer, plus the `Red is unstarted` / `Exec already ✅` warnings this step's skip/guard call needs. Honest-empty without `.kdbp/`; where the server is unregistered, read PLAN.md and its PLAN.json mirror directly. No test-shaped proof possible → the skip/guard decision (spec §Skip codes) — **never a fake red**. No test-shaped proof possible → the skip/guard decision (spec §Skip codes) — **never a fake red**.
2. **Scan the corpus** (E4): on a command-center project ask `mcp__gabe-map__cases_for` which C-ids already cover the touched subject (its `corpus.next_cid_floor` rides along) and `mcp__gabe-map__touches` which tests reach it — both honest-empty without a center — THEN grep existing C-ids + search tests related to the touched behavior: the map is a floor, the corpus is the registry. Print the `Searched:` line — an empty one invalidates the pass. Where the project carries a graft index, run `graft build && rm -f .ignore` (unconditional; ~2 s warm — **the `rm` is mandatory**, the build re-admits graft's own cards to ripgrep and poisons every later grep) and take the cases' **reach** with `mcp__gabe-map__who_calls` (the same two-arm core with the same gated emit), never `ask`; the by-hand `graft callers` + `graft grep` pair is the fallback where the server is unregistered, and `reach-emit.py` prints the record form.
