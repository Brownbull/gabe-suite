...
| S20 | element forms — the committed `forms.json` (the opt-in `_a3_paths` pass: what each FastAPI endpoint decides — every refusal, declared vs produced exits, the guards) carries ≥3 NAG findings across endpoints: `shared-status` (one status, different text-only refusals — a client must compare strings) · `reason-lost` (a service's reason replaced on the way out) · `escape-500` (a project error escaping untranslated) · `http-swallowed` (a refusal raised inside a broad except that returns normally). The COUNT class — `text-only`, `undeclared` — rides the line and never fires it: on a codebase that never codes a refusal every endpoint carries both. No forms.json → silent (the arm is opt-in); `present: false` → unavailable with the pass's reason; unreadable → silent | `mcp__gabe-map__map_census kind=forms` | committed `docs/site/center/forms.json` (`scripts/form_drift.py`, the same module review's FORM DRIFT runs with `--diff`) |
