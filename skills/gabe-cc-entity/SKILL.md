...
3. Locate the center: `docs/site/center/` found up from CWD, or `--center DIR` — `mcp__gabe-map__map_status` performs that same walk-up (for `center.config.json`) and adds the map's entities and freshness when the server is registered; `--center DIR` has no tool equivalent (the tool resolves a project ROOT). If no `center.config.json` anchor exists, this project has no built center — **STOP and route to `/gabe-cc-init`** (E6), never fabricate a pack.
