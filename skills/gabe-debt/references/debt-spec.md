...
`gabe-teach` uses "gravity wells" to mean **architectural sections / learning anchors** stored in `.kdbp/KNOWLEDGE.md` (legacy — retired from the default KDBP inventory in A2; `/gabe-teach` tracks wells only when a project still carries one; soft cap 7, Miller's number). Those are *where* the architecture lives.
...
Walk `.kdbp/SCOPE.md` (including its `## Phases` section — phase arc, status, and Covers REQs live there post-A2), `.kdbp/DECISIONS.md`, `.kdbp/PLAN.md`, `.kdbp/PENDING.md`, `templates/tier-sections/` index (if any bound via active phase's `types: []`). If a legacy `.kdbp/ROADMAP.md`, `.kdbp/KNOWLEDGE.md`, or `.kdbp/ENTITIES.md` exists (current `.kdbp/` or archived under `.kdbp/archive/retired/` — all retired from the default KDBP inventory in A2), walk those too. Apply the pattern's `## Detection — doc pass` heuristics. Classify each red-line question:
...
- **`/gabe-teach`** — a teach session for a gravity well (KNOWLEDGE.md entry, legacy — see gabe-teach's KNOWLEDGE.md status note) can surface R-rules that apply to that well as "already-paid lessons."
