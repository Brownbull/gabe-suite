...
| ANGLES | one REASON line per absent angle ("not yet mapped/run", never 'untested'-sounding words), ENDING IN A VERDICT: either "Worth growing: <what adding tests would buy, at what cost>" or "Nothing (further) to gain: <why>" — the renderer chips these (GROWTH OPPORTUNITY / NO GAIN — SETTLED) and flags a missing verdict visibly. Notes on partial angles render as footnotes | justifying instead of recording; reasons with no call |
