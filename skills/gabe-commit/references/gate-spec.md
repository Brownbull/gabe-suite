...
Alongside CHECK 1-9, run the five skill-local scripts (each exit 2 = WARN finding, never a block): `scripts/size-budget.sh` (>800-line budget), `scripts/evidence-freshness.sh` (Evidence Doctrine §4 — a proof-carrying phase must have proof_root artifacts at least as new as the staged source; WARNs append to `.kdbp/archive/evidence-bypass.log`), `scripts/docs-budget.sh` (documentation diet — new md outside allowed homes / dated-name md), `scripts/checkpoint-trailer.sh <message-file|->` (the execute task record, ruling 2026-08-07 — a message carrying a `Task:` footer is a checkpoint commit and must carry valid `Cases:` + `Class:` lines; run it against the composed message BEFORE committing), and `scripts/map-deltas.py analyze --sweep` (the map↔grep delta loop's tally sweep, 11a — UPSERTS the live deltas into the edge-keyed `(gen,subject,file)` tally ledger `.kdbp/map-deltas-rollup.jsonl` (dedup + `count`, nothing deleted), then digests the ACTIVE edges with a `consider: <gen>` line once an arm crosses the breadth threshold; exit 2 = a digest printed — fold it into the Notable Updates line. pulse S14 nags the standing tally between commits. The emit side runs at `/gabe-red` · `/gabe-execute` · `/gabe-review`; honest-empty on a non-`.kdbp` or delta-free tree — silent exit 0). Convention text: `../../gabe-docs/references/evidence-doctrine.md`; the delta loop's design record: `../../../docs/design/map-delta-loop/README.md`. (`append --once` is the writer-side dedupe both `reach-emit.py` and `mcp__gabe-map__who_calls` use — an identical un-swept edge is never re-appended)
...
**Optional `results_out:` key** in `## Verify Commands` (design record D1/D4 — report, never gate). A single path or a YAML list of paths (ruling R4 — multi-corpus repos emit one report per corpus, e.g. api/web/e2e junit files). Honored ONLY when the author wrote it beside a tests command THEY configured to emit a machine report there (e.g. `--junitxml=…`). NEVER append a reporter flag to a resolved command — that guesses a CLI surface and could fail a commit for a documentation feature. When present and CHECK 3 ran: write a small COMMITTED digest beside each listed path (`<path>.digest.json` — command, exit, passed/failed/skipped counts, duration, `partial: true` ONLY when the run stopped early — the resolved command carries `-x`/`--maxfail`/`--bail`, or the report counts fewer tests than were collected — never on a non-zero exit alone: a complete run with failures digests as `partial: false`, `failed: N` (gustify P8, 2026-09-04 — a full run had been stamped partial), HEAD sha, UTC timestamp). A listed report the tests run did not refresh (mtime older than the run's start) is NOT digested — name it `⤫ stale(not refreshed this run)` rather than digesting yesterday's numbers as today's. Committed, not gitignored: a gitignored run-manifest was once cleaned away and every doc page reverted to "not run" — the manifest is the memory; if the reports dir itself is gitignored by design, the opt-in edit adds a `!<dir>/*.digest.json` negation so the digest stays committable. Absent the key: emit nothing; downstream renders `⤫ skipped(no reporter)`. **This digest reports at every tier and gates at none.**
...
7. **Auto-tick Commit column in PLAN.md** (never silent: on mismatch print `ℹ PLAN: commit tick skipped (no-plan | phase-not-found | legacy-format | column-missing | footer-mismatch | red-checkpoint)` — one line, non-blocking). Only runs when the `git commit` in step 6.2 returned 0.
   - Follow the shared procedure documented in `/gabe-plan` under "Shared: auto-tick phase column" — including its precondition 5 (footer cross-check), its `RED:`-trailer precondition (a red checkpoint is never the phase's commit → `red-checkpoint`, gustify P8 2026-09-04) and its step 4b, which also mirrors the tick into `.kdbp/PLAN.json`; this command does not restate that logic
