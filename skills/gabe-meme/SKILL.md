...
2. **Draft oblique concepts.** For each meme: pick a template (never repeat one in the same set), write text that goes by the TANGENT of the fact, apply the mental-step test, and cap each text field at ~10 words. Use the generation prompt in `references/meme-craft.md` §Writing.
...
Per request: N verified meme PNGs (each rendered, Read-confirmed, non-repeating templates), each with a one-line caption naming its source when a real subject is involved, plus the concept table the human approved. No unseen renders, no punch-down, no template used against its persona. The template catalog, URL-encoding gotchas, brevity/legibility rules, and the writing prompt live once in `references/meme-craft.md`.
