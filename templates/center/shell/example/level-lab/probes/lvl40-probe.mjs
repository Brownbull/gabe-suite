...
ok(await T('window.__lvltest.icons()')==='lucide', 'Lucide is the default set (operator round 48)');
...
// lucide signature present at boot
const hasLucide = await pg.evaluate(() => [...document.querySelectorAll('#canvas path')]
  .some(p=>(p.getAttribute('d')||'').startsWith('M3 5v14')));
ok(hasLucide, 'Lucide database barrels drawn at boot');
...
await pg.click('#gear'); await pg.click('#icoSeg button[data-ico="classic"]');
...
ok(luc.set==='classic' && luc.cyl, 'Classic set still reachable (settled cylinder drawn)');
...
ok(await pg.evaluate(() => [...document.querySelectorAll('#canvas path')]
  .some(p=>(p.getAttribute('d')||'').startsWith('M3 5v14'))), 'Lucide restores as the default');
ok(await T('window.__lvltest.icons()')==='lucide', 'hook agrees');
