...
| `_a3_forms_build.py` + `_a3_forms_ids.py` · `_a3_forms_settings.py` · `_a3_forms_catch.py` · `_a3_forms_reach.py` · `_a3_forms_mw.py` · `_a3_forms_paths.py` · `_a3_forms_schema.py` · `_a3_forms_short.py` · `_a3_forms_switch.py` · `_a3_forms_walk.py` · `_a3_forms_effects.py` · `_a3_forms_contract.py` | element FORMS amendment 1 — the GENERATION ARMS. `_a3_forms_build.extend_backend` stamps `forms.json` version 2 + `head`, then runs the arms `center.config.json#forms_arms` / `GABE_FORMS_ARMS=a,b|all|none` select — all OFF by default, run per PART in `_a3_forms.ARM_STAGES`, each part only when its own hard needs succeeded, each stage isolated (a raise — or a feed the write could not serialize — restores the feed and reads `present:false` + reason), runners reading a private archmap copy; the orchestrator diffs each stage to know what an arm added, so a needed-only arm is computed in memory and stripped and every selected arm reports its `bytes`. Once any arm is on it stamps `x:` ids on every produced exit and `g:` ids (+ `exit`) on every precondition before the first stage, with an `ids` block. The leaves every arm shares: row ids with no line numbers, ranked feed-wide (`ids`) · `self.<attr>` → the settings fields behind it, env names and one-return properties expanded (`settings`) · the catch vocabulary with the full handler trail (`catch`) · the depth-capped reach walk that skips vendored center files (`reach`). The arms so far: `_a3_forms_mw` (kinds — middleware + dependency forms) · `_a3_forms_paths` (paths — returns · deciding branches · collapsed calls · path conditions · body-parse exits) · `_a3_forms_schema` (short — request schemas + 422 cases, over the data-only `_a3_forms_short`) · `_a3_forms_switch` (switches — binding · value · flag) · `_a3_forms_walk` (paths — one path per exit) · `_a3_forms_effects` (effects — steps along each path, read against the point it leaves each function · failure catches · race facts) · `_a3_forms_contract` (contract — the idempotency key and what it claims · auth schemes, gates, requires, provisions · rate limits resolved to settings · each exit's response shape). Batteries: `tests/forms-core` · `tests/forms-paths` · `tests/forms-kinds` · `tests/forms-short` · `tests/forms-effects` · `tests/forms-contract` · plan: `docs/design/element-forms/amendment-1.md` |
