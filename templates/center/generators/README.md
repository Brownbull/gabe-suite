...
| `_a3_forms_build.py` + `_a3_forms_ids.py` · `_a3_forms_settings.py` · `_a3_forms_catch.py` · `_a3_forms_reach.py` | element FORMS amendment 1 — the GENERATION ARMS. `_a3_forms_build.extend_backend` stamps `forms.json` version 2 + `head`, then runs the arms `center.config.json#forms_arms` / `GABE_FORMS_ARMS=a,b|all|none` select — all OFF by default, run per PART in `_a3_forms.ARM_STAGES`, each part only when its own hard needs succeeded, each stage isolated (a raise — or a feed the write could not serialize — restores the feed and reads `present:false` + reason), runners reading a private archmap copy; the orchestrator diffs each stage to know what an arm added, so a needed-only arm is computed in memory and stripped and every selected arm reports its `bytes`. The leaves every arm shares: row ids with no line numbers (`ids`) · `self.<attr>` → the settings fields behind it, env names and one-return properties expanded (`settings`) · the catch vocabulary with the full handler trail (`catch`) · the depth-capped reach walk that skips vendored center files (`reach`). Battery: `tests/forms-core` · plan: `docs/design/element-forms/amendment-1.md` |
