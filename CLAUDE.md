...
- **The codebase-graph station** is emitter + shell machinery, not an authored page: `templates/center/generators/_a3_graph.py` derives a LIBRARY-NEUTRAL C4 `{nodes,edges}` view from the in-memory archmap (FK-only L1 edges, zero new source read) → committed `c4-graph.json` + the `window.GABE_C4`/`GABE_C4_COLORS` sibling `c4-graph.js`; `templates/center/shell/codebase-graph.html` renders it as the change-simulation lifecycle instrument (principal top-nav · 4 stage lenses · explodable typed pieces · per-piece lifecycle) through the shared `templates/center/shell/assets/sim-panel.js` (`window.GABE_SIM_PANEL`, loaded by the station AND the arch-graph lab, so it is edited once). Honest-empty at rest: `build_center_a3` writes `sim.data.js = window.GABE_SIM = null` (only-if-absent), so a real twin shows the plain map + "no change in flight" until C2's `_a3_sim.py` derives a live projection from inflight+archmap+git+junit+PENDING. Batteries: `tests/arch-graph` (emitter) + `tests/codebase-graph` (station — static/structural, guards the class the render gates can't see: a `hidden` element defeated by an author `display:`).
