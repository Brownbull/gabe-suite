...
| **gabe-pulse** | 1.6.0 | Read-only completeness sweep — ten deterministic signals in four bands, plus the **ANGLE family** (`scripts/angles.py`): TEN evidence-backed triggers naming which satellite would find something now (S8 = workflow-census capture debt · S9 = entity-shape drift, an orphan URL domain or a cross-cutting aspect modeled as a domain, computed fresh from `archmap.json` by `scripts/entity_shape.py` — nothing stored to go stale · S10 = web-bridge drift, fetches that named no declared endpoint at emit time, read from the committed `c4-graph` by `scripts/fetch_bridge.py`), printed as ONE line at the end of every spine beat so the 15 manual-only skills stop depending on memory; reset patterns are record-tight (prose collisions measured 2026-08-07), the diff source sees past `.kdbp` bookkeeping commits, S1's command is pasteable verbatim; decay record, stated kill condition, pulse-angles + entity-drift batteries |
