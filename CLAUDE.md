...
| **gabe-pulse** | 1.5.0 | Read-only completeness sweep — ten deterministic signals in four bands, plus the **ANGLE family** (`scripts/angles.py`): NINE evidence-backed triggers naming which satellite would find something now (S8 = workflow-census capture debt · S9 = entity-shape drift, an orphan URL domain or a cross-cutting aspect modeled as a domain, computed fresh from `archmap.json` by `scripts/entity_shape.py` — nothing stored to go stale), printed as ONE line at the end of every spine beat so the 15 manual-only skills stop depending on memory; reset patterns are record-tight (prose collisions measured 2026-08-07), the diff source sees past `.kdbp` bookkeeping commits, S1's command is pasteable verbatim; decay record, stated kill condition, pulse-angles + entity-drift batteries |
...
| **gabe-review** | 1.17.0 | Code review — risk pricing, confidence scoring, plan alignment, triage; case-estate subjects (NEW CASE/BUMP/DRIFT, reserved C-ids) + absent-angle GROWTH triage (cap 7) + WORKFLOW DRIFT (census vs diff: census-lag · claim-drift · capture-debt) + REACH DRIFT (unreached · unused reach, vs the phase's `Reach:` record) + ENTITY-SHAPE DRIFT (a diff-added route landing in a URL domain no entity owns, vs the model — `entity_shape.py --diff`) + STALE ANCHOR (5c on PENDING: cited files moved past a row's `Verified` sha) on the same pricing |
