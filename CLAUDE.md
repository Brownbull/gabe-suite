...
| **gabe-cc-init** | 1.3.0 | Brownfield command-center adoption — archive-never-delete init, machine-ranked shortlist + the URL-domain SECOND LENS (advisory `entity_shape.py` view: orphan domains → candidate entities · aspect entities the ranking missed), one section per run at human speed, walk-recorded approval; tracker lives outside PLAN.md (human-initiated only) |
...
| **gabe-commit** | 2.7.3 | Commit quality gate — deterministic checks incl. size-budget + the checkpoint task-record trailer (scripts/checkpoint-trailer.sh: a `Task:`-footered message must carry valid `Cases:`/`Class:`), triage, simplify tier, docs-audit; optional results_out digest, path or list (reports every tier, gates none) |
| **gabe-docs** | 1.4.1 | Documentation standards + diagrams library + the suite execution contract incl. the E8 beat tail (stated once, all six spine skills point at it) (background) |
...
| **gabe-execute** | 2.6.2 | Phase execution with tier cap, escalation gate, checkpoint commits; the task RECORD is a commit-message trailer (`Cases:`/`Class:` beside the `Task:` footer, validated by /gabe-commit — the printed TASK CONTRACT retired 2026-08-07 after going 0-for-19) + `Reach:` (recorded, never binding — scoping to it is a defect) + case-scoped verify; narration legs authored hot |
| **gabe-cc-update** | 1.7.2 | Command-center feature coverage — card/diagrams/narration over machine facts; verdicts RENDERED from review triage (authored fallback); closes the PLAN `Center` cell on review; status, backfill, curate, release; bootstrap pointer → /gabe-cc-init |
...
| **gabe-init** | 2.3.3 | Project setup — `.kdbp/`, the 9 KDBP hooks, project type, maturity (human-initiated only) |
