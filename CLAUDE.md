...
| **gabe-commit** | 2.5.0 | Commit quality gate — deterministic checks incl. size-budget, triage, simplify tier, docs-audit; optional results_out digest, path or list (reports every tier, gates none) |
...
| **gabe-execute** | 2.4.0 | Phase execution with tier cap, escalation gate, checkpoint commits; TASK CONTRACT carries the phase's `CASES:` (C-ids from /gabe-red) + a `Reach:` line (recorded, never binding — scoping to it is a defect) + case-scoped verify; narration legs authored hot |
...
| **gabe-init** | 2.3.1 | Project setup — `.kdbp/`, hooks, project type, maturity (human-initiated only) |
