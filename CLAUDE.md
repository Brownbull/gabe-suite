...
|---|---|---|
| **gabe-artifact** | 1.3.0 | House chrome for published Artifacts — left-anchored content, one cog panel top-right, font roster + three suite skins (Catalog/Blueprint/Mission Console, light+dark), iconed section title pills (condensed grotesque, caps, the page's ONLY distinctive title face), a 12px legibility floor measured at the smallest base, H4 motion-first; 36-check render gate + 19-case chrome battery + a generic motion gate with a 6-case battery; two pattern libraries (20 motion + 31 static) |
...
| **gabe-commit** | 2.4.1 | Commit quality gate — deterministic checks incl. size-budget, triage, simplify tier, docs-audit; optional results_out digest, path or list (reports every tier, gates none) |
...
| **gabe-execute** | 2.3.1 | Phase execution with tier cap, escalation gate, checkpoint commits; TASK CONTRACT carries the phase's `CASES:` (C-ids from /gabe-red) + case-scoped verify; narration legs authored hot |
...
| **gabe-imagine** | 1.3.1 | Give it a context (concept · app · use case · phenomenon · code map) → the I0–I5 flow: dissect (seats/spheres/gravity worlds, `references/dissection/` BOUND), propose representations ranked critical/recommended/nice-to-have + ASK (I2 checkpoint), index for multi-session coverage, produce under the surface contract (component-led sections, ≤150 surface words, register captions, assets lifted before invented), gate & stop. Five grammars by material shape; four states; actor-table STOP. **Two targets:** `disk` (default, command-center pages + `{{PRISM:}}` fragments) · `artifact` (published URL). Gates: contract (8) + fit (6-case battery) + motion + render probe |
