...
check('var _JRNLEVELS={' in page and 'Orientation' in page and 'Core' in page and 'Specialized' in page and 'level:(w.level>=1&&w.level<=3)?w.level:0' in page,
...
check('function _stepData(n){' in page and 'function _dataHTML(sd){' in page and 'function _sideHTML(arr){' in page and 'function _fieldsHTML(sd){' in page and 'class="jdentdot"' in page and 'class="jdclu"' in page and 'class="jdopc"' in page and 'class="jdfrom"' in page and 'class="jdto"' in page and '"fnwrites","writes_to"' in page and 'op="guards"; opk="gate"; note="evaluates "' in page,
      "the journey-detail entity/cluster columns, the OPERATION column, or the deterministic FROM / TO data columns (op/targets/fields/gate condition) are gone")
...
check('class="jdcols"' in page and '#uni-jrnref .jdcols, #uni-jrnref .jdstep{ display:grid;' in page and 'background:var(--panel);' in page and 'padding:0 18px 22px' in page and 'class="jdgrp"' not in page and 'style="--ec:' in page and 'var(--ec, transparent) 9%' in page and 'function _srcsOf(id, rels){' in page and 'function _feEndpoints(id){' in page,
      "the journey-detail GRID layout (header+rows share one template) / the FLUSH opaque sticky header (body padding-top:0) / entity-tinted rows / the incoming-edge (FROM) resolver are gone, or the retired entity group rows came back")
...
(async()=>{
  ...
  const jrnDetail = await p.evaluate(() => {
    ...
    const r0=all[0], rc=[r0.querySelector('.jdnum'),r0.querySelector('.jdico'),r0.querySelector('.jdmain'),r0.querySelector('.jdent'),r0.querySelector('.jdclu'),r0.querySelector('.jdopc'),r0.querySelector('.jdfrom'),r0.querySelector('.jdto')].map(e=>e?Math.round(e.getBoundingClientRect().left):-1);
    ...
    return { clickable, nsteps, steps, groups, icons, stepsMatch:(steps===nsteps), walkedToStep:(closed && walkI===2),
      entDots, clus, ops, tgts, fields, gates, noHOverflow, cols, colsSticky, tinted, opcs, flows, fromTo, flush, aligned };
  }).catch(e=>({err:String(e)}));
  await b.close();
  ...
  const jd=jrnDetail, jrnDetailOk = jd && !jd.err && jd.clickable && jd.steps>0 && jd.stepsMatch && jd.groups===0 && jd.cols===1 && jd.colsSticky && jd.tinted===jd.steps && jd.opcs===jd.ops && jd.flows===jd.steps && jd.fromTo>0 && jd.icons===jd.steps && jd.walkedToStep
    && jd.entDots===jd.steps && jd.clus===jd.steps && jd.ops>0 && jd.tgts>0 && jd.fields>0 && jd.noHOverflow && jd.flush && jd.aligned;
