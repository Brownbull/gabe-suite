...
(async()=>{
  ...
  // D-038 (2026-09-23): ?node=<id> selects ONE node and opens its card — FIRES on a real endpoint, stays SILENT on an id the feed
  // lacks. Run LAST: selecting frames the camera on the node, which the earlier cases (wires, badge slots) must not inherit.
  const nodeDL = await p.evaluate(() => { try{ var ep=nodes.find(function(x){ return x.kind==='endpoint' && x.__threeObj; }); var r={want:ep?ep.id:null};
      SEL=null; window.__node=r.want; window.__nodeDone=false; __uniApplyDeepLinks(); r.got=(SEL&&SEL.data&&SEL.data.id)||null; r.card=/USAGE|Usage/.test((document.getElementById('pbody')||{}).textContent||'');
      SEL=null; window.__node='endpoint:NOPE /no-such-route-zz'; window.__nodeDone=false; __uniApplyDeepLinks(); r.bogus=(SEL&&SEL.data&&SEL.data.id)||null; window.__node=null; return r; }catch(e){ return {err:String(e)}; } }).catch(e=>({err:String(e)}));
  console.log('  nodeDL '+JSON.stringify(nodeDL));
  await b.close();
  ...
  const nodeOk = !!(nodeDL && !nodeDL.err && nodeDL.want && nodeDL.got===nodeDL.want && nodeDL.card===true && nodeDL.bogus===null);
  ...
  const ok = r.nodes>0 && !r.err && errs.length===0 && r.cardOpen && r.stPass && feOk && wireSelOk && scaleOk && nodeOk && dispOk && legend56Ok && homingOk && fewOk && wfOk && iconsOk && hdrOk && jrnOk && levelsOk && commitsOk && ui3Ok && fcbOk && focusOk && keyOk && legRefOk && jrnDetailOk && storeOk && modelOk;
