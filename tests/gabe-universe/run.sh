...
check('backend:["endpoint","function","schema","model","external","entity","middleware","flag","provider","prompt"]' in page and page.count('"external","entity","middleware","flag","provider","prompt"]')>=3 and 'middleware:"a check that runs before' in page and 'provider:"an outside service' in page,
...
(async()=>{
  ...
  const storeCheck = await p.evaluate(() => {
    ...
    return { journey:j.name, storeCols, storeFlagOn, litStore, iconInline, titleHot, cellClean, row0Lit, anchorKind, anchorKind3, lgbiOk, drafts:drafts.length, draftWalk, draftGrp, draftLeveled:drafts.every(x=>x.level>=1&&x.level<=3), draftNamed:drafts.every(x=>/^(Look at|Add|Edit|Remove|Manage) /.test(x.name||'')), viewType, viewCol, viewIcon };
  }).catch(e=>({err:String(e)}));
  await b.close();
  ...
  const sc=storeCheck, storeOk = sc && !sc.err && sc.storeCols>=1 && sc.storeFlagOn && sc.litStore>=1 && sc.iconInline && sc.titleHot && sc.cellClean && sc.row0Lit>=0 && sc.row0Lit<=5 && sc.anchorKind==='route' && sc.anchorKind3==='route' && sc.lgbiOk===true && sc.drafts>=1 && sc.draftWalk>0 && sc.draftGrp && sc.draftLeveled && sc.draftNamed && sc.viewType==='View (FE)' && sc.viewCol==='#d946ef' && (sc.viewIcon==='heat'||sc.viewIcon==='#d946ef');
